...
class CppMicroServicesConan(ConanFile):
    ...
    def package(self):
        ...
        for bundle in self.cppmicroservices_bundles:
            ...
            src_path = "{bundle}/src".format(bundle=bundle)
            build_include_path = "{build_dir}/{bundle}/include".format(build_dir=self.build_dir, bundle=bundle)
            self.copy('*.h', dst=include_path, src=include_path)
            self.copy('*.h', dst=include_path, src=src_path)
            self.copy('*.h', dst=include_path, src=build_include_path)

        # Third party headers
        self.copy('*.h', dst='third_party', src='third_party')
        ...
        self.copy('*.so*', dst='lib', src=lib_dir) # In unix systems, the version number is appended
    ...
    def package_info(self):
        ...
        self.cpp_info.libs = ['usHttpService', 'usShellService', 'CppMicroServices', 'usWebConsole']
